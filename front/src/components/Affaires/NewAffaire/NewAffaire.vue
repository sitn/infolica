...
export default {
  ...
  methods: {
    ...
    callInitMap: function () {
      ...
      this.$refs.mapHandler.initMap(defaultCenter, process.env.VUE_APP_MAP_DEFAULT_ZOOM);
    ...
    onMapClick: function(_this){
      return function(evt) {
        ...
        if(evt.coordinate){
          ...
          _this.$refs.mapHandler.addMarker(_this.form.localisation_E, _this.form.localisation_N);
        }
      }
    },
    ...
    */
    initPostData () {
      ...
      if(this.form.client_commande_par_id)
        formData.append("client_commande_par_id", this.form.client_commande_par_id.id);      
