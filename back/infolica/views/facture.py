...
""" Add new facture_emolument"""
@view_config(route_name='facture_emoluments', request_method='POST', renderer='json')
@view_config(route_name='facture_emoluments_s', request_method='POST', renderer='json')
def facture_emoluments_new_view(request):

    model = models.EmolumentFacture()
    model = Utils.set_model_record(model, request.params)
    # récupère le prix unitaire de l'élément
    prix_unitaire = request.dbsession.query(models.TableauEmoluments).filter(models.TableauEmoluments.id==model.emolument_id).first().montant
    # calcule le montant des éléments sélectionnés
    model.montant = int(model.nombre) * float(prix_unitaire) * float(model.facteur_correctif)
