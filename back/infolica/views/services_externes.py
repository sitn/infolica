...
def service_by_id_view(request):
    ...
    record = request.dbsession.query(Service).filter(Service.id == service_id).first()
...
def services_view(request):
    ...
    onlyprincipal = request.params["onlyprincipal"] == "true" if "onlyprincipal" in request.params else False
    ...
    if onlyprincipal is True:
        records = request.dbsession.query(Service).filter(Service.service_principal == True)
...
def services_update_view(request):
    ...
    if not record:
        raise CustomError(CustomError.RECORD_WITH_ID_NOT_FOUND.format(Service.__tablename__, service_id))
